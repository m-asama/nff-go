...
func ulHandler(pkt *packet.Packet, ctx flow.UserContext) bool {
	pkt.ParseL3()
	ipv4 := pkt.GetIPv4()
	if ipv4 == nil {
		return false
...
func dlHandler(pkt *packet.Packet, ctx flow.UserContext) bool {
	pkt.ParseL3()
	ipv4 := pkt.GetIPv4()
	if ipv4 == nil {
		return false
...
func main() {
	...
	flow.CheckFatal(flow.SetHandlerDrop(ulFlow, ulHandler, nil))
	flow.CheckFatal(flow.SetHandlerDrop(dlFlow, dlHandler, nil))
